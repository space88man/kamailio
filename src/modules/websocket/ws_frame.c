	...
	switch(opcode) {
		...
		case OPCODE_BINARY_FRAME:
			if(likely(frame.wsc->sub_protocol == SUB_PROTOCOL_SIP)) {
				...
				/* mode 2: dispatch the MSRP payload to a worker (tagged), which
				 * runs the SREV_TCP_MSRP_FRAME decode - composes with Step 15.
				 * The worker sources the conn id from rcv->proto_reserved1. */
				if(unlikely(ksr_tcp_main_threads == 2)) {
